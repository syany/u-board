...
public final class UraKeyboardApplication extends Application {
    ...
    public void start(Stage stage) throws Exception {
        ...
        final List<UraBlackKey> blackKeyList = newArrayList();
        ...
        for (int note = FIRST_NOTE, wIdx = 0; note < END_NOTE; note++) {
            ...
            if (UraKeyboardUtils.isBlack(note)) {
                ...
                .x((w + WHITE_KEY_WIDTH / 2) + BLACK_KEY_OFFSET).y(0).width(BLACK_KEY_WIDTH).height(BLACK_KEY_HEIGHT).note(note);
                blackKeyList.add(blackKey);
                keyboardList.add(blackKey);
            } else {
                final UraWhiteKey whiteKey = new UraWhiteKey(uraReceiver)
                    .x(wIdx * WHITE_KEY_WIDTH).y(0).width(WHITE_KEY_WIDTH).height(WHITE_KEY_HEIGHT).note(note);
                whiteKeyList.add(whiteKey);
                keyboardList.add(whiteKey);
                wIdx++;
            }
        }
